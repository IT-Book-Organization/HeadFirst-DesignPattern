...
- `View` : 모델을 표현하는 방법을 제공. 표시할 때 필요한 상태와 데이터는 모델에서 직접 가져온다.
- `Controller` : 사용자로부터 입력을 받으며 입력받은 내용이 모델에게 어떤 의미가 있는지 파악한다.
- `Model` : 모델에는 모든 데이터, 상태와 애플리케이션 로직이 들어있다. 뷰와 컨트롤러에서 모델의 상태를 조작하거나 가져올 때 필요한 인터페이스를 제공하고, 모델이 자신의 상태 변화를 옵저버들에게 연락해주긴 하지만, 모델은 뷰와 컨트롤러에 관심이 없다.
